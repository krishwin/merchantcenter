...
import { useRouter } from 'next/router';
  ...
  const {subscriptionDetails,setSubscriptionDetails} = useState({});
   ...
   useEffect(() => {
      ...
      const result = await fetch(
			'https://irad6avdaqvzwto-subscriberdb.adb.us-phoenix-1.oraclecloudapps.com/ords/admin/restsub/manage/subscription/'+ subscription,
        {
          method: 'GET',
	  headers: {
 ...
 const [{month, year}, setDate] = useState({
  ...
  const handleDateChange = useCallback(
    ...
    setSelectedendDates(newValue.start+365);
    },
    [],
  );
  ...
  <Card title="Active dates" sectioned>
    <p>Start Date</p>
    <Button onClick={togglePopoverActive} >{moment(selectedDates).format('MM/DD/YYYY')}</Button>
      </Card>
